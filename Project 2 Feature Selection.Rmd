...
I am going to try running a full model (putting every single variables into the model) and then estimate the variable importance. From there, I'm going to subset
```{r, comment = ""}
library(rminer)
library(parsnip)
...
full <- glm(diabetes~HighBP+HighChol+CholCheck+BMI+Smoker+Stroke+heart_dis+PhysActivity+Fruits+Veggies+hvy_alc+AnyHealthcare+NoDocbcCost+GenHlth+MentHlth+PhysHlth+DiffWalk+Sex+Age+Education+Income, data=diabetes50, family=binomial(link="logit"))
summary(full)
# estimate variable importance
importance <- varImp(full, scale=FALSE)
